...
int pstopen(void **handle, const char* clientID, const char* serverURI, void* context)
{
	...
	perserverURI = (char*) malloc(strlen(serverURI) + 1);
	strcpy(perserverURI, serverURI);
	while ((ptraux = strstr(perserverURI, ":")) != NULL)
		*ptraux = '-' ;

	/* consider '/'  +  '-'  +  '\0' */
	clientDir = (char*) malloc(strlen(dataDir) + strlen(clientID) + strlen(perserverURI) + 3);
...
int pstput(void* handle, char* key, int bufcount, char* buffers[], int buflens[])
{
	...
	/* consider '/' + '\0' */
	file = (char*) malloc(strlen(clientDir) + strlen(key) + strlen(MESSAGE_FILENAME_EXTENSION) + 2 );
...
int containskeyWin32(char *dirname, char *key)
{
	...
	if (hDir != INVALID_HANDLE_VALUE)
	{
		while (!fFinished)
		{
			...
			{
				filekey = (char*) malloc(strlen(FileData.cFileName) + 1);
...
int containskeyUnix(char *dirname, char *key)
{
	...
	if((dp = opendir(dirname)) != NULL)
	{
		...
		{
			char* filename = (char*) malloc(strlen(dirname) + strlen(dir_entry->d_name) + 2);
			sprintf(filename, "%s/%s", dirname, dir_entry->d_name);
			lstat(filename, &stat_info);
			free(filename);
			if(S_ISREG(stat_info.st_mode))
			{
				filekey = (char*) malloc(strlen(dir_entry->d_name) + 1);
...
int clearWin32(char *dirname)
{
	...
	if (hDir != INVALID_HANDLE_VALUE)
	{
		while (!fFinished)
		{
			if (FileData.dwFileAttributes & FILE_ATTRIBUTE_ARCHIVE)
			{
				file = (char*) malloc(strlen(dirname) + strlen(FileData.cFileName) + 2);
...
int keysWin32(char *dirname, char ***keys, int *nkeys)
{
	...
	if (hDir != INVALID_HANDLE_VALUE)
	{
		...
		while (!fFinished)
		{
			if (FileData.dwFileAttributes & FILE_ATTRIBUTE_ARCHIVE)
			{
				fkeys[i] = (char*) malloc(strlen(FileData.cFileName) + 1);
...
int keysUnix(char *dirname, char ***keys, int *nkeys)
{
	...
	if((dp = opendir(dirname)) != NULL)
	{
		...
		{
			char* temp = (char*) malloc(strlen(dirname)+strlen(dir_entry->d_name)+2);
	...
	if (nfkeys != 0)
	{
		...
		if((dp = opendir(dirname)) != NULL)
		{
			...
			while((dir_entry = readdir(dp)) != NULL)
			{
				...
				if (lstat(temp, &stat_info) == 0 && S_ISREG(stat_info.st_mode))
				{
					fkeys[i] = (char*) malloc(strlen(dir_entry->d_name) + 1);
...
#define RC !rc ? "(Success)" : "(Failed) "
	...
	{
		key = (char*) malloc(MESSAGE_FILENAME_LENGTH + 1);
