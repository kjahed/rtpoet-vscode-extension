...
static void MQTTProtocol_storeQoS0(Clients* pubclient, Publish* publish)
{
	...
	pw = (pending_write*) malloc(sizeof(pending_write));
...
{
	Publications* p = (Publications*) malloc(sizeof(Publications));
	...
	*len += sizeof(Publications);
	...
	p->payload = (char*) malloc(publish->payloadlen);
