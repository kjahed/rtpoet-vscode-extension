...
void UMLRTCapsuleId::setCapsule( UMLRTCapsule * capsule_ )
{
    capsule = capsule_;
}

const char * UMLRTCapsuleId::getId() const
{
    return id;
}

void UMLRTCapsuleId::setId( const char * id_ )
{
    id = id_;
}
...
static int UMLRTObject_fprintf_UMLRTCapsuleId( FILE *ostream, const UMLRTObject_class * desc, const void * data, int nest, int arraySize )
{
    ...
    for (int i = 0; i < arraySize; ++i)
    {
        ...
        nchar += fprintf(ostream, " %s (global id: %s)}", array[i].getCapsule()->name(), array[i].getId());
    }
    return nchar;
}
