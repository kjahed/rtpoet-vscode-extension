...
public:
    ...
    // t_usage_option_help - option information to output informative feedback.
    typedef struct {
    ...
    static char * getAddressForCapsule ( const char * capsuleName );
private:

    // Print usage in response to -h or --help or invalid option.
    static void usage ( const char * const program );

    // Check option sanity - to compensate for any non-POSIX-compliant getopt_long implementations, as found
    // in g++ (Ubuntu 4.8.2-19ubuntu1) 4.8.2.
    static bool optionsValid ( t_usage_option_help options[], int argc, char * const * argv );

    static UMLRTHashMap * getCapsuleAddressMap();

    static bool parseCapsuleAddressLine ( char * line );

    static bool readCapsuleAddressMap( const char * deploymentFile );
    ...
    static UMLRTHashMap * capsuleAddressMap;
};
