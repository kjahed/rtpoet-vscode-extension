...
{
	char* colon_pos = (char*) strrchr(uri, ':'); /* reverse find to allow for ':' in IPv6 addresses */
	char* buf = (char*)uri;
	size_t len;
	...
	if (buf[len - 1] == ']')
	{
		if (buf == (char*)uri)
		{
			buf = (char*) malloc(len);  /* we are stripping off the final ], so length is 1 shorter */
