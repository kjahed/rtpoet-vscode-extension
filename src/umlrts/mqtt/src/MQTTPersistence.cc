...
int MQTTPersistence_create(MQTTClient_persistence** persistence, int type, void* pcontext)
{
	...
	switch (type)
	{
		...
		case MQTTCLIENT_PERSISTENCE_DEFAULT :
			per = (MQTTClient_persistence*) malloc(sizeof(MQTTClient_persistence));
			if ( per != NULL )
			{
				if ( pcontext != NULL )
				{
					per->context = malloc(strlen((char*)pcontext) + 1);
					strcpy((char*)per->context, (char*)pcontext);
				}
				else
					per->context = (void*) ".";  /* working directory */
...
int MQTTPersistence_restore(Clients *c)
{
	...
	if (c->persistence && (rc = c->persistence->pkeys(c->phandle, &msgkeys, &nkeys)) == 0)
	{
		while (rc == 0 && i < nkeys)
		{
			...
			{
				MQTTPacket* pack = (MQTTPacket*) MQTTPersistence_restorePacket(buffer, buflen);
				if ( pack != NULL )
				{
					...
					else if ( strstr(msgkeys[i],PERSISTENCE_PUBLISH_SENT) != NULL )
					{
						Publish* publish = (Publish*)pack;
						Messages* msg = NULL;
						char *key = (char*) malloc(MESSAGE_FILENAME_LENGTH + 1);
...
int MQTTPersistence_persistQueueEntry(Clients* aclient, MQTTPersistence_qEntry* qe)
{
	...
	FUNC_ENTRY;
	lens = (int*)malloc(nbufs * sizeof(int));
	bufs = (void**) malloc(nbufs * sizeof(char *));
...
static MQTTPersistence_qEntry* MQTTPersistence_restoreQueueEntry(char* buffer, size_t buflen)
{
	...
	FUNC_ENTRY;
	qe = (MQTTPersistence_qEntry*) malloc(sizeof(MQTTPersistence_qEntry));
	memset(qe, '\0', sizeof(MQTTPersistence_qEntry));
	
	qe->msg = (MQTTPersistence_message*) malloc(sizeof(MQTTPersistence_message));
	...
	qe->topicName = (char*) malloc(data_size);
