...
void* mymalloc(char* file, int line, size_t size)
{
	...
	if ((s->ptr = (int*) malloc(size + 2*sizeof(int))) == NULL)
	{
		Log(LOG_ERROR, 13, errmsg);
		free(s->file);
		free(s);
...
void *myrealloc(char* file, int line, void* p, size_t size)
{
	...
	else
	{
		...
		if (state.current_size > state.max_size)
			state.max_size = state.current_size;
		if ((s->ptr = (int*) realloc(s->ptr, size + 2*sizeof(int))) == NULL)
		{
			Log(LOG_ERROR, 13, errmsg);
			return NULL;
		}
...
int main(int argc, char *argv[])
{
	...
	h = (char*) realloc(h, 14);
	h = (char*) realloc(h, 25);
	h = (char*) realloc(h, 255);
	h = (char*) realloc(h, 2225);
	h = (char*) realloc(h, 22225);
    printf("freeing h\n");
	free(h);
	Heap_terminate();
	printf("Finishing\n");
