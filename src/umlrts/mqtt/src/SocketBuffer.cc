...
void SocketBuffer_newDefQ(void)
{
	def_queue = (socket_queue*) malloc(sizeof(socket_queue));
	def_queue->buflen = 1000;
	def_queue->buf = (char*) malloc(def_queue->buflen);
...
char* SocketBuffer_getQueuedData(int socket, size_t bytes, size_t* actual_len)
{
	...
	if (bytes > queue->buflen)
	{
		if (queue->datalen > 0)
		{
			...
			free(queue->buf);
			queue->buf = (char*) newmem;
		}
		else
			queue->buf = (char*) realloc(queue->buf, bytes);
		queue->buflen = bytes;
	}
...
#endif
{
	...
	pw = (pending_writes*) malloc(sizeof(pending_writes));
	pw->socket = socket;
#if defined(OPENSSL)
	pw->ssl = ssl;
#endif
