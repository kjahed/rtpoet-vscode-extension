		...
		int persistence_type, void* persistence_context)
{
	...
	m = (MQTTClients*) malloc(sizeof(MQTTClients));
	*handle = m;
	...
	m->c = (Clients*) malloc(sizeof(Clients));
...
{
	MQTTClients* m = (MQTTClients*) *handle;
...
static thread_return_type WINAPI MQTTClient_run(void* n)
{
	...
	while (!tostop)
	{
		...
		m = (MQTTClients*)(handles->current->content);
		if (m == NULL)
		{
			/* assert: should not happen */
			continue;
...
void Protocol_processPublication(Publish* publish, Clients* client)
{
	...
	mm = (MQTTClient_message*) malloc(sizeof(MQTTClient_message));
...
static int MQTTClient_connectURI(MQTTClient handle, MQTTClient_connectOptions* options, const char* serverURI)
{
	...
	if (options->will && (options->will->struct_version == 0 || options->will->struct_version == 1))
	{
		...
		m->c->will = (willMessages*) malloc(sizeof(willMessages));
	...
	{
		m->c->sslopts = (MQTTClient_SSLOptions*) malloc(sizeof(MQTTClient_SSLOptions));
	...
	else if (options->struct_version >= 5 && options->binarypwd.data)
	{
		...
		m->c->password = (char*) malloc(m->c->passwordlen);
							 ...
							 int qos, int retained, MQTTClient_deliveryToken* deliveryToken)
{
	...
	p = (Publish*) malloc(sizeof(Publish));
...
static MQTTPacket* MQTTClient_cycle(int* sock, unsigned long timeout, int* rc)
{
	...
	if (*sock > 0)
	{
		...
		if (m != NULL)
		{
			...
			{
				pack = (MQTTPacket*) MQTTPacket_Factory(&m->c->net, rc);
				if (*rc == TCPSOCKET_INTERRUPTED)
					*rc = 0;
			}
		}
...
void MQTTClient_yield(void)
{
	...
	do
	{
		...
		{
			MQTTClients* m = (MQTTClients*) (handles->current->content);
...
int MQTTClient_getPendingDeliveryTokens(MQTTClient handle, MQTTClient_deliveryToken **tokens)
{
	...
	*tokens = NULL;
	...
	if (m->c && m->c->outboundMsgs->count > 0)
	{
		...
		*tokens = (MQTTClient_deliveryToken*) malloc(sizeof(MQTTClient_deliveryToken) * (m->c->outboundMsgs->count + 1));
