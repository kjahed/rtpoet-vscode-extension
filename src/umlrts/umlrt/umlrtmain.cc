...
UMLRTHashMap * UMLRTMain::capsuleAddressMap = NULL;
...
static UMLRTMain::t_usage_option_help optionhelp[] = {
		...
		{ "deployment",    	'd', "<deployment-file>", "Specify a deployment file." },
...
/*static*/ void UMLRTMain::setArgs( int argc_, char * const * argv_ )
{
   static struct option options[] = {
			...
			{ "deployment",        	required_argument, NULL, 'd' },
    ...
    char * deploymentfile = NULL;
    int longindex = 0;
    bool optionsvalid = optionsValid(optionhelp, argc_, argv_);

    while (optionsvalid && ((optchar = getopt_long( argc_, argv_, ":hcd:D:C:T:t:N:S:n:F:L:f:m:Msul", options, &longindex)) != -1) && !userargsfound)
    {
    ...
    if (deploymentfile)
    {
        //TODO: find a better way to handle deployment
    	readCapsuleAddressMap(deploymentfile);
    }
    if (controllerfile)
    {
...
/*static*/ UMLRTHashMap * UMLRTMain::getCapsuleAddressMap()
{
    if (capsuleAddressMap == NULL)
    {
    	capsuleAddressMap = new UMLRTHashMap("capsuleAddressMap", UMLRTHashMap::compareString, false/*objectIsString*/);
    }
    return capsuleAddressMap;
}

/*static*/ char * UMLRTMain::getAddressForCapsule ( const char * capsuleName )
{
	return (char*)getCapsuleAddressMap()->getObject(capsuleName);
}

/*static*/ bool UMLRTMain::parseCapsuleAddressLine ( char * line )
{
    bool ok = false;
    char * capsuleName;
    char * saveptr;
    ...
    if (capsuleName != NULL)
    {
        ...
        if (address != NULL)
        {
        	getCapsuleAddressMap()->insert(capsuleName, (void *)address);
            ok = true;
        }
    }
    return ok;
...
/*static*/ bool UMLRTMain::readCapsuleAddressMap( const char * deploymentFile )
{
    bool ok = true;
    FILE *fd;

    if ((fd = fopen(deploymentFile, "r")) == NULL)
    {
        printf("ERROR: failed to open deploymentFile file %s\n", deploymentFile);
        ok = false;
    }
    else
    {
        ...
        {
            ok = parseCapsuleAddressLine(buf);
        }
    }
    return ok;
}
