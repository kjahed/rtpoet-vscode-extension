...
void UMLRTSignal::initialize ( const char * name, Id id, const UMLRTCommsPort * srcPort, const UMLRTObject * const object, void * data)
{
    if (element)
    {
        FATAL("initializing signal that already has an element allocated.");
    ...
    element->initialize(name, id, srcPort, object, data);
}
