...
// Un-comment the lines that correspond to the types you want enabled by default.
// Use command-line option -T to enable individual types from at run-time.
...
// See UMLRTMain.hh
bool debugEnableTypeSet( int bdType, bool enable)
{
    bool ok = true;
    if ((bdType < 0) || (bdType >= BD_MAXPLUS1))
    {
        ok = false;
    }
    else if (enable)
    {
        debugEnableMessageType |= (1 << bdType);
    }
    else
    {
        debugEnableMessageType &= ~(1 << bdType);
...
bool debugEnableTypeMaskNameSet(const char * name, bool enable)
{
    ...
    else
    {
        ...
        {
            debugEnableTypeSet(index, enable);
        }
    }
    return ok;
}
...
{
    return (debugEnable && (debugEnableMessageType & (1 << type)) != 0);
}
...
void debugOptionSummary()
{
    ...
    printf("Color terminal text (option 'debugcolor/C')  : %s (implementation disabled)\n",
            debugEnableColor ? "enabled" : "disabled");
